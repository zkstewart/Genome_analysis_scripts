...
def uniref_xml_parse(tableFile, xmlFile):
        ...
        xmlBlock = ['','','']      # Format is [accession, gene name, common taxon]
                        ...
                        if line.startswith('<entry id='):
                                if xmlBlock == ['','', '']:                                     # i.e., if this is the first loop, just start building the current block
                                        ...
                                        if xmlBlock[0] in accDict:
                                                ...
                                                if xmlBlock[2] == '':
                                                        xmlBlock[2] = 'None'
                                                accDict[xmlBlock[0]] = xmlBlock[1:]             # We just need to hold onto the gene name and taxon code
                                        # Start new block
...
with open(args.inputTable, 'r') as fileIn, open(args.outputFileName, 'w') as fileOut:
        for line in fileIn:
                ...
                if line.startswith('#Query\tSource'):
                        fileOut.write('#Query\tSource\tTarget_accessions\tGene_names\tNCBI_taxonomy_of_hits\tPercentage_identity\tAlignment_length\tMismatches\tGap_opens\tQuery_start\tQuery_end\tTarget_start\tTarget_end\tExpect_value\tBit_score\tBest_hit_with_idmapping\n')
                        ...
                        if line[2] == '.':
                                ...
                                for i in range(len(accs)):
                                        ...
                                        if i == 0:
                                                ...
                                                formattedTaxa += '[' + accDetails[1] + ']'
                                # Output
                                newL = [*line[0:3], formattedNames, formattedTaxa, *line[3:]]
                                fileOut.write('\t'.join(newL) + '\n')
