#! python3
# mikadocompare_novel_analyser
# Script to obtain "novel" loci as determined by mikado compare and obtain some
...
## gffcompare-related
def mikadocompare_tmap_parse(tmapFile, compGff3Index):
        # Setup
        novelLoci = []
        with open(tmapFile, 'r') as fileIn:
                for line in fileIn:
                        # Skip unnecessary lines
                        if line.startswith('ref_id\tref_gene') or line == '\n':
                                continue
                        # Extract details from line
                        sl = line.split('\t')
                        classCodes = sl[2].split(',')
                        mrnaID = sl[4]
                        for code in classCodes:
                                if code in ['x', 'X', 'p', 'P', 'u']:
        ...
        for loci in novelLoci:
                geneID = compGff3Index[loci]['attributes']['ID']
                if geneID not in geneIDs:
                        noisoNovelLoci.append(loci)
                geneIDs.append(geneID)
        print(str(len(noisoNovelLoci)) + ' novel loci were detected from TMAP parsing after isoform collapse.')
        return noisoNovelLoci
...
by mikado compare and what their likely classifications are e.g., if they are possibly
...
p.add_argument("-t", "-tmap", dest="tmapFile",
               help="Specify the mikado .tmap file.")
p.add_argument("-f", "-fasta", dest="fastaFile",
               help="Specify the FASTA file corresponding to the CDS regions of the 'comparison' file provided to mikado compare.")
p.add_argument("-r", "-refgff", dest="refGff3",
               help="Specify the GFF3 file used as the 'reference' file provided to mikado compare.")
p.add_argument("-c", "-compgff", dest="compGff3",
               help="Specify the GFF3 file used as the 'comparison' file provided to mikado compare.")
...
novelLoci = mikadocompare_tmap_parse(args.tmapFile, compGff3Index)
if args.novelLoci:
        for loci in novelLoci:
                print(loci)
        quit()
